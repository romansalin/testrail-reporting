...
class MainReport(object):
    def __init__(self):
        ...
        self.users = {d.pk: '{0} ({1})'.format(d.name, d.email)
                      for d in Users.objects.only('id', 'name', 'email')}
        self.projects = {d.pk: d.name
                         for d in Projects.objects.only('id', 'name')}
        self.milestones = {d.pk: d.name
                           for d in Milestones.objects.only('id', 'name')}
        self.suites = {d.pk: d.name for d in Suites.objects.only('id', 'name')}
        self.priorities = {d.pk: d.name
                           for d in Priorities.objects.only('id', 'name')}
        self.sections = {d.pk: d.name
                         for d in Sections.objects.only('id', 'name')}
        self.case_types = {d.pk: d.name
                           for d in CaseTypes.objects.only('id', 'name')}
        self.plans = {d.pk: d.name for d in Plans.objects.only('id', 'name')}
        self.status = {d.pk: d.label
                       for d in Statuses.objects.only('id', 'label')}
        self.cases = {d.pk: d.title
                      for d in Cases.objects.only('id', 'title')}
        self.runs = {d.pk: d.name
                     for d in Runs.objects.only('id', 'name')}
        self.tests = {d.pk: d.title for d in Tests.objects.only('id', 'title')}
